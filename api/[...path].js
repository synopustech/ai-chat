...
export default async function handler(req) {
  ...
  const pathname = url.pathname; // e.g. /api/v1/chat/completions, /api/search, /api/voice/tts

  let target;

  if (pathname.startsWith('/api/v1/')) {
    // /api/v1/* → api.synopustech.com/v1/*
    target = `https://api.synopustech.com${pathname.replace('/api/v1/', '/v1/')}${url.search}`;
  } else if (pathname === '/api/search') {
    // /api/search → search.synopustech.com/search
    target = `https://search.synopustech.com/search${url.search}`;
  } else if (pathname.startsWith('/api/voice/')) {
    // /api/voice/* → voice.synopustech.com/api/*
